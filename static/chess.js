    ...
    for (i = 0; i < 8; i++) {
        for (j = 0; j < 8; j++) {
            ...
            if (board[i][j].innerHTML) {
                board[i][j].addEventListener('click', check(board[i][j]));
            } 
...
const moveWpawn = () => {
    for (i = 0; i < 8; i++) {
        ...
        if (poz.classList.contains("toggled")) {
            board[1 + 1][i].classList.add("move");
            board[1 + 2][i].classList.add("move2");
        }
        if (!poz.classList.contains("toggled")) {
            board[1 + 1][i].classList.remove("move");
            board[1 + 2][i].classList.remove("move2");
        }
    }
};
